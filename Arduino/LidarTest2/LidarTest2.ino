...
static uint16_t lastDegree = 0;
static boolean lastDegreeValid = false;
static boolean degreeErrorDetected = false;
...
void loop() {
  ...
  if (lastDegreeValid) {
    if ((lastDegree + 4)%360 != degree) {
      degreeErrorDetected = true;
      expected = 4 + lastDegree;
      found = degree;
      errorCounter++;
    }
  }
  ...
  for (uint8_t i=0; i<4; i++, degree++) {
    ...
    if (degree == 0) {
      ...
      if (digitalRead(PIN_DEBUG) == HIGH) {
          ...
          Serial.print(degreeData.signalStrength);
          Serial.print(" ");
      }
      else
      {
          ...
          if (degreeErrorDetected) {
            ...
            lastDegreeValid = false;
            degreeErrorDetected = false;
            errorCounter = 0;
          } else {
            Serial.println(" ");
          }
